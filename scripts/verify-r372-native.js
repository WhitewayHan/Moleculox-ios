...
if(!['ios','android'].includes(platform)) throw new Error('Usage: verify-r372-native.js ios|android');
const read=(p)=>fs.readFileSync(path.join(root,p),'utf8');
const must=(c,m)=>{if(!c)throw new Error(m)};
const idx=read('www/index.html');
const game=read('www/js/game-r160.js');
const firebase=read('www/js/firebase.js');
must(idx.includes("window.__MX_BUILD_ID__='8.7.204-r372-science-legends-"+platform+"';"),'wrong native build id');
...
must(game.includes('const APP_VERSION="v8.7.204 · R372 SCIENCE LEGENDS";'),'R372 visible version mismatch');
...
console.log('R372 '+platform+' native source verification PASS');
