...
ok(game.includes("boardNeedsRealtime()?16.5"),'interactive board remains ~60fps target');
ok(game.includes("PARTS.length?(performanceLow()?33:16)"),'automatic pressure keeps active particle FX cadence at ~60fps');
...
ok(css.includes('R79 / R5 — ZERO-LOSS MOBILE PERFORMANCE PASS'),'R5 zero-loss CSS guard present');
ok(css.includes('.screen:not(.on):not(.mxLeaving)'),'hidden screens are suspended only when invisible');
const r5=css.slice(css.indexOf('R79 / R5 — ZERO-LOSS MOBILE PERFORMANCE PASS'));
