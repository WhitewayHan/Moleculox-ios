...
async function getOnlinePlayerCount() {
  ...
  try {
    ...
    // Multiple tabs or devices logged into the same Firebase account count as
    // one player. Anonymous guests still have a distinct UID per installation.
    const activeUsers = new Set();
    snap.forEach((row) => {
      const data = row.data() || {};
      if (typeof data.uid === "string" && data.uid) activeUsers.add(data.uid);
    });
